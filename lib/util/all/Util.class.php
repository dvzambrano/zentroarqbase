...
class Util {
    ...
    public static function getTextFromHTML($text) {
        // importat!!! this was added to prevent block writing formation on html
        if($text && trim($text) != ""){
    ...
    public static function getSummaryFromHTML($html, $query, $size = false, $hilight = false) {
        ...
        // getting text without HTML
        $content = Util::getTextFromHTML($html);
...
//        $percent = rand(0, 100);
    ...
    public static function getKeyWords($text = '', $exclude = array(), $amount = false) {
        // getting text without HTML
        $text = Util::getTextFromHTML($text);
...
//            return $text;
    ...
    public static function getBundle($name, $languaje = 'es-ES', $replacements = array()) {
