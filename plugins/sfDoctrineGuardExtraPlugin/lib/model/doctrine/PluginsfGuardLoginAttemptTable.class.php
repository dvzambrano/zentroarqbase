...
class PluginsfGuardLoginAttemptTable extends Doctrine_Table {
    ...
    public function isLockedOut($userIP) {
        ...
        $loginattempts = Util::getMetadataValue('app_unsuccessfulloginattempts');
        if (!$loginattempts)
            $loginattempts = sfConfig::get('app_sf_guard_extra_plugin_login_attempts', 10);

        $lockaccountfor = Util::getMetadataValue('app_lockaccountfor');
        if (!$lockaccountfor)
            $lockaccountfor = sfConfig::get('app_sf_guard_extra_lock_for', 600);



        $lastLoginAttempt = $result->getFirst()->getCreatedAt();
        $lockUntil = strtotime($lastLoginAttempt . ' + ' . $lockaccountfor . ' seconds');

        if ($result->count() < $loginattempts)
            return false;
        else
            return date('Y-m-d H:i:s', strtotime($lastLoginAttempt) + $lockaccountfor);
    }
