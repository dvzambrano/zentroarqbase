...
define("DB_HOST", $_GET['server']);
define("BACKUP_DIR", $_GET['directory']); // Comment this line to use same script's directory ('.')
define("BACKUP_FILE", $_GET['file']); // Script will autodetect if backup file is gzipped based on .gz extension
define("CHARSET", 'utf8');
        ...
        try {
            ...
            if ($backupFileIsGzipped) {
                if (!$backupFile = $this->gunzipBackupFile()) {
                    $this->obfPrint("ERROR: couldn't gunzip backup file " . $backupDir . '/' . $backupFile);
                }
            }
            ...
            if ($handle) {
                        ...
                        if (!$lineIsComment) {
                            ...
                            if (preg_match('/;$/', $line)) {
                                ...
                                if (mysqli_query($this->conn, $sql)) {
                                    if (preg_match('/^CREATE TABLE( IF NOT EXISTS)? `([^`]+)`/i', $sql, $tableName)) {
                                        $this->obfPrint("Table succesfully created: `" . $tableName[2] . "`");
                                    }
                                    $sql = '';
                                } else {
                                    $this->obfPrint("ERROR: SQL execution error: " . mysqli_error($this->conn));
                                }
                            }
                        }
                ...
                $this->obfPrint("ERROR: couldn't open backup file " . $backupDir . '/' . $backupFile);
            }
        } catch (Exception $e) {
