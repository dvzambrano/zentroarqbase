        ...
        init : function(App) {
            ...
            this.topToolbar = new Ext.Toolbar({
                ...
                new Ext.ux.ThemeCycleButton({
                    ...
                    listeners: {
                        click: function(button, eventObject) {
                            App.printView('/db/report', ' ', ' ');
                        }
                    }
                },{
                    iconCls: Ext.ux.Icon('email_open_image'),
                    listeners: {
                        click: function(button, eventObject) {
                            App.window = App.showWindow(bundle.getMsg('app.form.generaldata'), 620, 400, App.mailFormPanel, 
                                function(button, eventObject, callback){
                                    if(!button){
                                        button = new Object;
                                        button.id = App.window.submitBtn.id;
                                    }
                                    ...
                                    new Ext.data.Connection().request({
                                        ...
                                        params: {
                                            sendto: App.mailFormPanel.toField.getValue(),
                                            subject: App.mailFormPanel.subjectField.getValue(),
                                            partial: 'mail/NotificationSuccess',
                                            params: '[{"content":"'+App.mailFormPanel.msgField.getValue()+'"}]'
                                        },
                                        method: 'POST',
                                        callback : function(options, success, response) {
                                            mask.hide();
                                            ...
                                            if(json.success){
                                            // Ext.Base.msg('', String.format(bundle.getMsg('app.msg.info.sendmail.successful'), mailline[step].subject, mailline[step].sendto)); 
                                            }
                                            else
                                                Ext.Base.msg('', String.format(bundle.getMsg('app.error.sendmail.fail'), mailline[step].subject, mailline[step].sendto)); 
                                        
                                            if(button.id == App.window.submitBtn.id){
                                                App.mailFormPanel.getForm().reset();
                                                App.window.hide();
                                            }
                                        }
                                    });
                    ...
                    listeners: {
                        click: function(button, eventObject) {
                            var submitFn = function(button, eventObject, callback){
                                App.regCodeFormPanel.getForm().submit({
                                    waitTitle : bundle.getMsg('app.msg.wait.title'), 
                                    waitMsg: bundle.getMsg('app.msg.wait.text'), 
                                    clientValidation: true,
                                    success: function(form, action) {
                                        App.regCodeFormPanel.codeField.setValue(action.result.key);
                                        App.regCodeFormPanel.installedField.setValue(action.result.extra.installed);
                                        App.regCodeFormPanel.periodField.setValue(action.result.extra.given);
                                        App.regCodeFormPanel.expireField.setValue(action.result.extra.expire);
                            ...
                            var cancelFn = function(){
                                App.regCodeFormPanel.codeField.reset();
                                App.window.hide();
                            };

                            App.window = App.showWindow(bundle.getMsg('app.form.generaldata'), 700, 160, App.regCodeFormPanel, 
                                submitFn,
                                cancelFn,
                                button.getEl(),
                                false,
                        ...
                        //                                title, width, height, items, acceptFn, cancelFn, animateTarget, manageable, printFn, menuConfig, hideApply
                        }
                    }
                }/*deletetohere*/]
            });
