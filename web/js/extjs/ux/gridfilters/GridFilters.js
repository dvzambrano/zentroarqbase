...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    /** @private */
    init : function (grid) {
        if (grid instanceof Ext.grid.GridPanel || grid instanceof Ext.ux.tree.TreeGrid) {
            this.grid = grid;
...
*/
    ...
    reload : function (params) {
        if (this.local) {
            ...
            if (this.grid instanceof Ext.ux.tree.TreeGrid){
                var values = new Array();
                this.filters.each(function (filter) {
                    if (filter.active) {
                        var obj = new Object;
                        obj.type = filter.type;
                        obj.value = filter.getValue();
                        obj.field = filter.inputItem.dataIndex;
                        values.push(obj);
                    }
                });
                this.grid.getLoader().baseParams.filter = Ext.encode(values);
                this.grid.getLoader().load(this.grid.getRootNode());
