...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
    ...
    getView : function() {
        ...
        this.view.hmenu = this.hmenu;
        if(!this.view.cm)
            this.view.cm = new Object;
        this.view.cm.config = this.columns;
        this.view.hdCtxIndex = this.hdCtxIndex;
        this.view.mainHd = this.mainHd;
        return this.view;
    },
    ...
    getNodeData: function(item, columns, data) {                     
        var convertedData = [];
        ...
        for(var i=0;i<item.childNodes.length;i++)
            data = this.getNodeData(item.childNodes[i], columns, data);
    ...
    getHTML: function() {        
        ...
        var aux = [];
        Ext.each(columns, function(column) {
            if (column.header =='' || column.header =='id'){}
            else aux.push(column);
        }, this);
        columns = aux;
        var data = new Array();
        for(var i=0;i<this.getRootNode().childNodes.length;i++){
            var item = this.getRootNode().childNodes[i]; 
            data = this.getNodeData(item, columns, data);    
        }
